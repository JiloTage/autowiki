...
description: Customize the Auto-Wiki visual theme. Use when the user wants color, font, dark mode, Mermaid theme, or graph styling changes across assets/css/wiki.css, assets/js/graph.js, templates, and existing articles when required.
---
...
| `templates/article.html` | Mermaidテーマ設定 |
| `templates/index.html` | （テーマ依存の変更があれば） |
| 既存の `articles/*.html` | Mermaidテーマ変更時のみ一括更新 |
...
3. `templates/article.html` を読み込む（Mermaid設定）
...
`templates/article.html` の `mermaid.initialize()` 内:
```javascript
...
Mermaidテーマ変更時は既存の `articles/*.html` も一括更新。
